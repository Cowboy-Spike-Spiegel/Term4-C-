    ...
    ui(new Ui::MainWindow)
{
    ...
    person = new User(strList.at(0).toInt(), strList.at(1), user, strList.at(2).toDouble(),
                      strList.at(3),strList.at(4),strList.at(5));

    // 获取所有用户名单并顺便进行发送名单的生成
    str = "getUsers";
    ...
    for(int i=0; i < strList.count(); i++) {
        ...
        if(strList.at(i).toInt() != curAccount)   // 顺便生成发送名单
            ui->comboBox_sendUsers->addItem(strList.at(i));
    }
...
void MainWindow::refreshReceive() {
    // 进行指针的清理
    for(unsigned int i=0; i < vector_waitReceive.size() ; i++) {
        delete vector_waitReceive[i];   // 重点，基类指针释放子类********************************************||**
        vector_waitReceive[i] = nullptr;
    }
    vector_waitReceive.clear();

    // 查询所有未接收记录 -------------------------虚函数
    query.exec("select * from delivery_improve where receiver = "+QString::number(curAccount)+" and goodstate = '1'");
    int count = 0;
    while(query.next()) {
        qDebug() << query.value(9).toInt() << CourierNames.size();
        // 错误记录，直接跳过
        ...
        if(query.value(6).toInt() == 8*query.value(5).toInt()) {
            Good* obj = new Good_Fragile;   // 记得释放内存
        ...
        // 普通快递
            ...
            Good* obj = new Good_Normal;    // 记得释放内存
            obj->create(query.value(0).toInt(), query.value(1).toInt(), query.value(2).toInt(), query.value(3).toString(), query.value(4).toString(),
                        query.value(5).toInt(), query.value(7).toInt(), query.value(8).toString(), query.value(9).toInt());
            vector_waitReceive.push_back(obj);
        }
...
{
    QString sender, time_s, time_r, s, cost, courier;
    sender  = QString("%1 ").arg(Names.at(vector_waitReceive[index]->getAccountSender()-1),4,QLatin1Char(' '));
    ...
    cost    = QString(" %1  ")
            .arg(QString::number(vector_waitReceive[index]->getPrice()*vector_waitReceive[index]->getWeight(),'f',2),5,QLatin1Char(' '));
    if(vector_waitReceive[index]->getCoutier() == 0) courier = "      ";
    else courier = QString("%1 ").arg(CourierNames.at(vector_waitReceive[index]->getCoutier()-1),4,QLatin1Char(' '));

    return sender + time_s + time_r + s + cost + courier + vector_waitReceive[index]->getDiscription();
}
...
void MainWindow::refreshSend() {
    ...
    ui->comboBox_sendItem->setCurrentIndex(-1);
    ui->comboBox_sendWeight->setCurrentIndex(-1);
...
void MainWindow::on_pushButton_send_clicked()
{
    ...
    int weight = ui->comboBox_sendWeight->currentIndex()+1;
    if(receiver == 0 || weight == 0) {
        QMessageBox mess(QMessageBox::Information,"Warning!","No one selected or weight=0!");
        mess.exec();
        return;
    }
    // 修正账号number
    ...
    // 易碎品
        if(ui->comboBox_sendItem->currentIndex() == 0) {
            Good* obj = new Good_Fragile;
            obj->create(curAccount, receiver, weight, discription);
            if(!person->send(obj)) {
        ...
        // 书本
        else if(ui->comboBox_sendItem->currentIndex() == 1) {
        ...
        // 普通快递
        else if(ui->comboBox_sendItem->currentIndex() == 2) {
...
void MainWindow::refreshFind() {
    for(unsigned int i=0; i < vector_all.size(); i++) {
        delete vector_all[i];
        vector_all[i] = nullptr;
    }
    vector_all.clear();

    // 查询所有相关记录
    query.exec("select * from delivery_improve where sender = "+QString::number(curAccount)
...
QString MainWindow::argAllStr(int index) {
    ...
    cost    = QString(" %1  ")
            .arg(QString::number(vector_allSelect[index]->getPrice()*vector_allSelect[index]->getWeight(),'f',2),5,QLatin1Char(' '));
    if(vector_allSelect[index]->getCoutier() == 0)   courier = "      ";
    else courier = QString("%1 ").arg(CourierNames.at(vector_allSelect[index]->getCoutier()-1),4,QLatin1Char(' '));

    return number+sender+receiver+time_s+time_r+s+cost+courier+vector_allSelect[index]->getDiscription();
}
// 查询操作相关
...
void MainWindow::on_pushButton_update_clicked()
{
    ...
    // 修正为不用修改
    if(name == "输入名称" || name.isEmpty()) name = "%";
    if(tel == "输入电话" || tel.isEmpty()) tel = "%";
    if(address == "输入地址" || address.isEmpty()) address = "%";
    if(password == "输入密码" || password.isEmpty()) password = "%";
    ...
    if(name.contains(" ") || password.contains(" ") || tel.contains(" ") || address.contains(" ")) {
        ...
        mess.exec();
        return;
    } else if(!tel.isEmpty() && tel.length() != 11 && tel != "%") {
...
void MainWindow::setUI() {
    ...
    ui->label_recTitle->setText("发送方     发送时间           接收时间            状态    费用  快递员  描述");
    ...
    ui->comboBox_sendItem->setFont(QFont("Microsoft YaHei",20,75)) ;
    ui->comboBox_sendWeight->setStyleSheet("background: rgba(255,255,255,50%)") ;
    ui->comboBox_sendWeight->setFont(QFont("Microsoft YaHei",20,75)) ;
    ui->comboBox_sendItem->setStyleSheet("background: rgba(255,255,255,50%)") ;
    ...
    ui->label_allTitle->setText("单号  发送方   接收方    发送时间         接收时间       状态   费用   快递员   描述");
