    ...
    ui(new Ui::MainWindow_Administrator)
{
    ...
    if(tempFile.open(QIODevice::ReadOnly)) {
        ...
        admin = new Administrator(0, tmpList.at(1), administrator, tmpList.at(2).toDouble(),
    ...
    if(tempFile.open(QIODevice::ReadOnly)) {
        ...
        for(int i=0; i<strList.count(); i++) {
            ...
            ui->comboBox_courier->addItem(tmpList.at(3));   // 顺便生成快递员选项
        }
        // 清理
        strAll.clear();
        strList.clear();
...
void MainWindow_Administrator::refreshShow() {
    ui->label_name->setText(admin->getName());
    ui->label_account->setText(QString::number(admin->getAccount()));
    ui->label_tel->setText(admin->getTel());
    ui->label_balance->setText(QString::number(admin->getBalance(),'f',6));
}
...
void MainWindow_Administrator::setUI() {
    ...
    // 展示所有快递信息
    vector_goodSelect = vector_good;    // 初始化为选择所有
    refreshFindDelivery();
    ui->comboBox_deliverySelect->setCurrentIndex(-1);
